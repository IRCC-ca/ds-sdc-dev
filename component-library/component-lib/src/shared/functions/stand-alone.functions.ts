...
import { FormComponentConfig } from "../interfaces/form-component-config";
...
export class StandAloneFunctions {
   ...
   */
  getErrorState(config: FormComponentConfig): boolean {
    return (config.formGroup.get(config.id)?.touched &&
      config.formGroup.get(config.id)?.invalid) ?? false;
  }
}
